...
export default function MarketplacePage() {
  ...
  const filteredListings = listings.filter(item => {
    if (!selectedCategories.includes("All Categories") && !selectedCategories.includes(item.category)) {
      return false;
    }
    
    const priceStr = item.priceLabel ? item.priceLabel.replace(/[^0-9]/g, '') : '';
    const price = priceStr ? parseInt(priceStr, 10) : 0;
    
    if (maxPrice < 10000 && price > maxPrice) {
      return false;
    }
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-[#0a0a0c] flex flex-col font-sans">
      ...
      <div className="bg-white dark:bg-zinc-950 border-b border-zinc-200/80 dark:border-zinc-800/80 shadow-sm overflow-x-auto hide-scrollbar">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex items-center gap-6">
          <span className="text-sm font-extrabold uppercase tracking-widest text-foreground whitespace-nowrap">
            All Categories <ChevronDown className="inline h-4 w-4 ml-1" />
          </span>
          {["Academics", "Electronics", "Appliances", "Vehicles", "Furniture", "Miscellaneous"].map(cat => (
            <button 
              key={cat}
              onClick={() => toggleCategory(cat)}
              className={`text-sm font-semibold whitespace-nowrap transition-colors ${selectedCategories.includes(cat) ? 'text-primary' : 'text-zinc-600 dark:text-zinc-400 hover:text-primary'}`}
            >
              {cat}
            </button>
          ))}
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full flex flex-col lg:flex-row px-4 md:px-8 py-10 gap-10">
            ...
            <div className="space-y-8">
                  ...
                  {["All Categories", "Academics", "Electronics", "Appliances", "Vehicles", "Furniture"].map((cat) => (
                    <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                      <div className="relative flex items-center justify-center">
                        <input 
                          type="checkbox" 
                          className="peer appearance-none w-5 h-5 rounded-lg border-2 border-zinc-300 dark:border-zinc-700 checked:border-primary checked:bg-primary transition-all cursor-pointer" 
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full h-2 bg-zinc-200 dark:bg-zinc-800 rounded-full appearance-none cursor-pointer accent-primary mb-4" 
                />
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-200/80 dark:border-zinc-800/80 rounded-xl px-4 py-2 text-xs font-bold w-full text-center text-foreground">
                    Max: {maxPrice >= 10000 ? "₹10k+" : `₹${maxPrice}`}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="flex-1">
          ...
          <div>
            <h2 className="text-2xl font-black text-foreground mb-6">Fresh recommendations</h2>
